...
const imgPh = (label, size = '480×300') =>
  wt(`<div class="wt-img"><div class="wt-img-ph">
    <span>🖼️</span><p>${label}</p><small>권장: ${size}px</small>
  </div></div>`);
const narr   = txt  => wt(`<div class="wt-narr"><p>${txt}</p></div>`);
const speech = (spkr, txt) => wt(`<div class="wt-speech">
  <div class="wt-speech-name">${spkr}</div>
  <div class="wt-speech-txt">${txt}</div>
</div>`);
const secHdr = (num, title, sub) => wt(`<div class="wt-sec-hdr">
  <div class="wt-sec-num">${num}</div>
  <div class="wt-sec-title">${title}</div>
  <div class="wt-sec-sub">${sub}</div>
</div>`);
const data  = html  => wt(`<div class="wt-data">${html}</div>`);
const trait = (t, b) => `<div class="trait"><div class="trait-t">${t}</div><div class="trait-b">${b}</div></div>`;

/* ─ 메인 렌더 ─ */
function renderResult() {
  const raw = localStorage.getItem('sajuData');
  if (!raw) { window.location.href = 'index.html'; return; }
  const { year, month, day, hour, name, gender } = JSON.parse(raw);
  ...
  panels.push(`<div class="wt result-cover">
    ...
    <p class="rc-name">${name} · ${year}.${String(month).padStart(2,'0')}.${String(day).padStart(2,'0')}</p>
  ...
  panels.push(speech('루루', `${name}의 일주를 보니까<br/><strong>${char.name}</strong>이야.<br/>${char.intro}`));
  const traitLabels = ['💖 기본 성격','💅 행동 스타일','🌿 생활 패턴','🔮 속마음','✨ 숨겨진 매력'];
  panels.push(data(char.traits.map((t, i) => trait(traitLabels[i], t)).join('')));
  panels.push(data(`
  ...
  panels.push(data(`<div class="trait-t" style="margin-bottom:6px">🌟 나의 귀인</div>
  ...
  panels.push(data(['👶 초년 귀인','🌸 청년 귀인','💎 중년 귀인','👑 말년 귀인'].map(lbl => {
  ...
  panels.push(speech('루루', `${name},<br/><br/>갸루는 단순한 스타일이 아니야.<br/>자기만의 매력을 당당하게 보여주는 사람이야.<br/><br/>앞으로도 너답게 반짝여.`));
  panels.push(`<div class="wt finale">
    <span class="fin-icon">☽</span>
    <div class="fin-msg"><strong>GYARU FORTUNE REPORT</strong><br/><br/>by 루루</div>
    <button class="restart-btn" onclick="window.location.href='index.html'">처음으로</button>
  </div>`);

  /* ─ DOM에 삽입 ─ */
  document.getElementById('result-phase').innerHTML = panels.join('');

  // 오행 바 애니메이션
  setTimeout(() => {
    document.querySelectorAll('.el-fill').forEach(el => {
